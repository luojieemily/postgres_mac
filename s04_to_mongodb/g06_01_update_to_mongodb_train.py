...
def create_new_to_mongodb_train_table():
    ...
    qry = "CREATE INDEX idx_train ON " + \
        tb_to_mongodb_train+" (stations_ja, dep, arr);"
    cur.execute(qry)
    connection.commit()
...
def update_cols_from_dict_train():
    ...
    for each in r:
        ...
        qry = "update "+tb_to_mongodb_train+" set route_id='"+route_id+"', railwaylines=array" + \
            str(railwaylines)+", serviceroutes=array"+str(serviceroutes) + \
            ", train_type='"+train_type + \
            "' where stations_ja=array"+str(stations)
        print(qry)
        cur.execute(qry)
        connection.commit()
...
def update_cols_others():
    ...
    for each in results:
        ...
        qry = "update "+tb_to_mongodb_train+" set stations_en=array"+str(stations_en)+", stations_zh=array"+str(stations_zh)+", depm="+str(
            depm)+", arrm="+str(arrm)+" where stations_ja=array"+str(stations_ja)+" and dep=array"+str(dep)+" and arr=array"+str(arr)
        print(qry)
        cur.execute(qry)
        connection.commit()
...
def change_special_character():
    qry = r"select detail_info, stations_ja, dep, arr from "+tb_to_mongodb_train + \
        " where array_to_string(detail_info,',') like '%\\u3000%'"
    cur.execute(qry)
    r = cur.fetchall()
    for each in r:
        detail_info = each[0]
        ...
        qry = "update "+tb_to_mongodb_train+" set detail_info = array" + \
            str(new_detail_info)+" where stations_ja=array"+str(stations_ja) + \
            " and dep=array"+str(dep)+" and arr=array"+str(arr)
        print(qry)
        cur.execute(qry)
        connection.commit()
...
def update_icon_image():
    ...
    cur.execute(qry)
    connection.commit()
    qry = "select train_type, detail_info from " + \
        tb_to_mongodb_train + " group by train_type, detail_info"
    cur.execute(qry)
    results = cur.fetchall()
    for each in results:
        train_type = each[0]
        ...
        qry = "update "+tb_to_mongodb_train+" set icon_image='"+icon_image + \
            ...
            "' and detail_info=array"+str(detail_info)
        print(qry)
        cur.execute(qry)
        connection.commit()
