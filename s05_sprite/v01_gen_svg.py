    ...
    for d in direction:
        ...
        with open(os.getcwd()+'/svg/'+icon_image+'.svg', 'w') as f:
            f.write('<svg width="'+str(icon_width)+'" height="' +
                    str(icon_width)+'" xmlns="http://www.w3.org/2000/svg">')
            ...
            f.write('<ellipse stroke-width="0" ry="'+str(icon_width/2-icon_width/20)+'" rx="'+str(icon_width/2-icon_width /
                    20)+'" id="svg_2" cy="'+str(icon_width/2)+'" cx="'+str(icon_width/2)+'" fill="'+inner_color+'" />')
            f.write('<text font-size="'+str(icon_width/1.8)+'"  font-style="normal" font-weight="900" x="'+str(icon_width/2) +
                    '" y="'+str(icon_width/1.9)+'" text-anchor="middle"  fill="white" dy="'+str(icon_width/6)+'" >'+word+'</text>')
            f.write(' </g>')
            f.write('</svg>')
