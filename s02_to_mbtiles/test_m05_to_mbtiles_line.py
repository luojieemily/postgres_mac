...
for t in types:
    geojson_file_name=geojson_path+'/'+t+'_'+config['version_code']+'.geojson'
    layer_data+=' -L '+t+":"+geojson_file_name
    
mbtiles_file_name=mbtiles_path+'/line_'+config['version_code']+'.mbtiles'
            
# subprocess.call('tippecanoe -o '+mbtiles_file_name+' -z 16 --force '+ geojson_file_name, shell=True, cwd=mbtiles_path)
subprocess.call('tippecanoe -o '+mbtiles_file_name+' --drop-densest-as-needed --extend-zooms-if-still-dropping --force '+ layer_data, shell=True, cwd=mbtiles_path)
