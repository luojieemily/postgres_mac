...
to_file_path=config['geojson_DIRECTORY']
to_file_name='point_vip_'+config['version_code']+'.geojson'
...
qry = "SELECT jsonb_build_object('type',  'FeatureCollection', 'features', jsonb_agg(feature))  FROM (SELECT jsonb_build_object('type', 'Feature', 'geometry',  ST_AsGeoJSON(geog, 6)::jsonb,   'properties', to_jsonb(row)- 'geog' ) AS feature FROM (SELECT "+select_column+", ST_Transform( way, 4326) as geog  FROM "+tb_from +" where railway_line_ja is not null and priority <91999 order by priority ) row) features;"
cur.execute(qry)
