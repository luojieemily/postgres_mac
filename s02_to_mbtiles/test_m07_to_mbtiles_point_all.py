...
geojson_file_name=geojson_path+'/point_all_'+config['version_code']+'.geojson'
layer_data+=' -L point:'+geojson_file_name
    
mbtiles_file_name=mbtiles_path+'/point_all_'+config['version_code']+'.mbtiles'
            
# subprocess.call('tippecanoe -o '+mbtiles_file_name+' -z 16 --force '+ geojson_file_name, shell=True, cwd=mbtiles_path)
subprocess.call('tippecanoe -zg -Z7 -o '+mbtiles_file_name+' --coalesce-densest-as-needed --extend-zooms-if-still-dropping  --force '+ layer_data, shell=True, cwd=mbtiles_path)
